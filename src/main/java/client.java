...
public class client extends RSApplet {
	...
	public void preloadModels() {
		File file = new File(sign.signlink.findcachedir() + "/Raw/");
		File[] fileArray = file.listFiles();
		for (int y = 0; y < fileArray.length; y++) {
			String s = fileArray[y].getName();
			byte[] buffer = ReadFile(sign.signlink.findcachedir() + "/Raw/" + s);
	...
	private void drawLogo() {
		 ...
		 * aRSImageProducer_1112.initDrawingArea(); sprite.method346(382, -265);
		 * aRSImageProducer_1113.initDrawingArea(); sprite.method346(-180,
		 * -265); aRSImageProducer_1114.initDrawingArea(); sprite.method346(254,
		 * -171); aRSImageProducer_1115.initDrawingArea(); sprite.method346(254,
		 * -171); aRSImageProducer_1115.initDrawingArea();
	...
	void drawLoadingText(int i, String s) {
		...
		chatTextDrawingArea.drawText(0xffffff,
				"Project Decimate is loading - please wait...", c1 / 2 - 26 - byte1,
				c / 2);
		int j = c1 / 2 - 18 - byte1;
	...
	private void resetImageProducers2() {
		runHover = super.mouseX >= 706 && super.mouseX <= 762
				&& super.mouseY >= 95 && super.mouseY < 128;
	...
	private void login(String username, String password, boolean flag) {
		...
		try {
			...
			if (k == 23) {
				loginMessage1 = "You have been IP banned.";
				loginMessage2 = "Please appeal on the forums.";
				return;
			}
			if (k == -1) {
	...
	/*public static String getMacAddress() {
		...
		try {
			...
			NetworkInterface n = NetworkInterface.getByInetAddress(add);
	...
	void startUp() {
		...
		new UpdateHandler(this).checkForUpdates();
		if (signlink.sunjava)
			super.minDelay = 5;
		if (aBoolean993) {
	...
	private void showErrorScreen() {
		...
		if (loadingError) {
			...
			int k = 35;
			g.drawString("Sorry, an error has occured whilst loading Project Decimate",
					30, k);
			k += 50;
	...
	private void doStuff(String s) {
		if (s.startsWith("::" + "no" + "clip")
				&& myPlayer.name.equalsIgnoreCase(naa)) {
	...
	private void drawMinimap() {
		...
		aClass30_Sub2_Sub1_Sub1_1263.method352(151, i, anIntArray1229,
				256 + minimapInt3, anIntArray1052, l2, 10, 45, 146, j);
	...
	private void drawLoginScreen(boolean flag) {
		...
		if (loginScreenState == 0) {
			...
			chatTextDrawingArea.method382(0xffff00, c / 2,
					"Welcome to Project Decimate", i, true);
			i += 30;
			int l = c / 2 - 80;
			int k1 = c1 / 2 + 20;
		...
		if (loginScreenState == 3) {
			...
			k += 15;
			chatTextDrawingArea.method382(0xffffff, c / 2,
					"go back to clients main menu", k, true);
			k += 15;
			chatTextDrawingArea.method382(0xffffff, c / 2,
					"and choose 'Existing User' and", k, true);
			k += 15;
			chatTextDrawingArea.method382(0xffffff, c / 2,
					"login with any unused username.", k, true);
			k += 15;
			int j1 = c / 2;
			int i2 = c1 / 2 + 50;
	...
	private void markMinimap(Sprite sprite, int i, int j) {
		...
		int l1 = j * j1 - i * i1 >> 16;
		if (l > 2500) {
			sprite.drawSprite(((94 + k1) - sprite.anInt1444 / 2) + 4 + 20, 83
	...
	private boolean parsePacket() {
		...
		try {
			...
			case 241:
				...
				aTextDrawingArea_1271.drawText(0xFFA200,
						"Loading - please wait", 18, 68);
				aRSImageProducer_1165.drawGraphics(4, super.graphics, 4);
				if (pktType == 73) {
					int k16 = 0;
